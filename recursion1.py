...
def bunnyEars2(bunnies): # this is a list
    if bunnies == 0: # no bunies, no ears. base case
        return 0
    if (bunnies % 2 == 0):   # if the bunny number is an even number 
...
def strCount(str, sub):
    ...
    if(len(str) < subLength):
        return 0
    if str[0:subLength] == (sub):
        return 1 + strCount(str[subLength:],sub)
...
def strDist(str, sub):
    ...
    if(stLen < sbLen):
        return 0
    if(str[0:sbLen] == (sub)):
        if(str[stLen - sbLen:stLen] == (sub)):
            return stLen
        return strDist(str[0:stLen - 1], sub)
