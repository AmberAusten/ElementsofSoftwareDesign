...
def groupSum(start, nums, target): # start is an index
    ...
    if start >= len(nums):
        ...
        #return 1==2 or 1 == 1
                                                        ...
                                                        # [1,2,3,4,5,6] #nums #target 12
                                                        # choose 1 [2,3,4,5,6] = 12 - 1 or 11
...
def groupNoAdj(start, nums, target):
    # if you choose one, the one after it you cannot choose 
...
def groupSum5(start, nums, target): 
    ...
    # value after multiple of 5 is 1, cannot be chosen

    if start >= len(nums):
        return target == 0
    
    else:
        if nums[start] % 5 == 0:         # value after multiple of 5 is 1, cannot be chosen
            if start != len(nums) - 1:
                if nums[start + 1] == 1: 
...
def splitArrayHelper (nums,i,sums): 
    # nums is the array passed in of integers
    
    if i == len(nums):
        return sums == 0
    elif splitArrayHelper(nums, i+1, sums + nums[i]):
        return True 
    else:
        return splitArrayHelper(nums, i+1, sums - nums[i])
...
def splitOdd10Helper(nums,i, ten, odd):

	if i == len(nums):
		return (ten % 2 == 1 and odd % 10 == 0 or odd % 2 == 1 and ten % 10 == 0)
	if splitOdd10Helper(nums, i + 1, ten + nums[i], odd):
		return True
	return splitOdd10Helper(nums, i + 1, ten, odd + nums[i])
...
def split53Helper (nums,i,multiples):
	
    if i == len(nums):
        return multiples == 0
    if nums[i] % 3 == 0:
        return split53Helper(nums, i + 1, multiples - nums[i])
    if nums[i] % 5 == 0:
        return split53Helper(nums, i + 1, multiples + nums[i])
    if split53Helper(nums, i + 1, multiples + nums[i]):
        return True
    else:
        return split53Helper(nums, i + 1, multiples - nums[i])
