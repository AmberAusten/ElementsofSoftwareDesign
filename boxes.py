#  File: Boxes.py

#  Student Name: Amber

#  Date Created: 10/13

#  Date Last Modified: 10/16

# generates all subsets of boxes and stores them in all_box_subsets
# box_list is a list of boxes that have already been sorted
# sub_set is a list that is the current subset of boxes
# idx is an index in the list box_list
# all_box_subsets is a 3-D list that has all the subset of boxes
def sub_sets_boxes (box_list, sub_set, idx, all_box_subsets):
    hi = len(box_list)    # need a base case. I need to find the high, the length of a, the last index Plus 1 
    if idx == hi:         # means I have reached the end
        all_box_subsets.append(sub_set)
        return            # when a function returns, popped off the stack 
    else:                 # recursive case. first make a copy of my basket b
        c = sub_set[:]         # b is a list BTW, how do I make a copy of B? This makes a copy 
        sub_set.append (box_list[idx])
        sub_sets_boxes (box_list, sub_set, idx + 1, all_box_subsets)
        sub_sets_boxes (box_list, c, idx + 1, all_box_subsets)

# goes through all the subset of boxes and only stores the
# largest subsets that nest in the 3-D list all_nesting_boxes
# largest_size keeps track what the largest subset is
def largest_nesting_subsets (all_box_subsets, largest_size, all_nesting_boxes):
    # sort subsets by length 
  def length (b): 
    return len(b)

  all_box_subsets.sort(key = length)
  all_box_subsets.reverse()                                           # we want to start with the longest subset of boxes
    
  for i in range (len(all_box_subsets)):  
    all_fit = True                                                     # index, I am in in all box bsubsets, idx [0] = [56,67,78],[56,78,98], a list of boxes 
    for j in range (len(all_box_subsets[i])-1):                           # looking at one individual box j = [3,6,7] individual box
      #does_fit(all_box_subsets[i][j],all_box_subsets[i][j + 1])         # see if it nests with the one after it, record the length
      if not does_fit(all_box_subsets[i][j],all_box_subsets[i][j + 1]):
        all_fit = False 
        break
    if all_fit:
        # largest_size = max(largest_size, len(all_box_subsets[i]))
      largest_size = len(all_box_subsets[i])
      break 
  for i in range (len(all_box_subsets)):
    if len(all_box_subsets[i]) == largest_size:
      all_fit = True 
      for j in range (len(all_box_subsets[i])-1):                           # looking at one individual box j = [3,6,7] individual box
        if not does_fit(all_box_subsets[i][j],all_box_subsets[i][j + 1]):
          all_fit = False 
          break
      if all_fit: 
        all_nesting_boxes.append(all_box_subsets[i])
    elif len(all_box_subsets[i]) < largest_size:
      break
  return largest_size                                                # are there any other of the same length that also nest
  

# returns True if box1 fits inside box2
def does_fit (box1, box2):
  return (box1[0] < box2[0] and box1[1] < box2[1] and box1[2] < box2[2])

def main():
  # read the number of boxes 
  in_file = open ('boxes.in', 'r')
  line = in_file.readline()
  line = line.strip()
  num_boxes = int (line)

  # create an empty list for the boxes
  box_list = []

  # read the boxes from the file
  for i in range (num_boxes):
    line = in_file.readline()
    line = line.strip()
    box = line.split()
    for j in range (len(box)):
      box[j] = int (box[j])
    box.sort()
    box_list.append (box)

  # close the file
  in_file.close()

  
  # print to make sure that the input was read in correctly
  #print (box_list)
  #print()
  

  # sort the box list
  box_list.sort()


  # print the box_list to see if it has been sorted.
  #print (box_list)
  #print()
  

  # create an empty list to hold all subset of boxes
  all_box_subsets = []

  # create a list to hold a single subset of boxes
  sub_set = []

  # generate all subsets of boxes and store them in all_box_subsets
  sub_sets_boxes (box_list, sub_set, 0, all_box_subsets)
  #print(len(all_box_subsets))

  

  # initialize the size of the largest sub-set of nesting boxes
  largest_size = 0

  # create a list to hold the largest subsets of nesting boxes
  all_nesting_boxes = []

  # go through all the subset of boxes and only store the
  # largest subsets that nest in all_nesting_boxes
  #largest_nesting_subsets (all_box_subsets, largest_size, all_nesting_boxes)

  # print the largest number of boxes that fit
  # The first line will be an integer giving the largest number of boxes that can fit inside each other.
  largestNumberBoxes = largest_nesting_subsets (all_box_subsets, largest_size, all_nesting_boxes)
  print (largestNumberBoxes)

  # print the number of sets of such boxes
  
  print (len(all_nesting_boxes))

if __name__ == "__main__":
  main()
